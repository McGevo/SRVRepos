...
namespace SRV.Models
{
    class Unit 
    {
        ...
        public Unit(string subjectCode, string subjectDesc, string grade)
        {
        ...
        public List<Unit> SelectUnits(string email)
        {
            ...
            query = "SELECT subject.SubjectCode, SubjectDescription, Grade FROM student_grade INNER JOIN crn_detail ON student_grade.CRN = crn_detail.CRN INNER JOIN subject ON crn_detail.SubjectCode = subject.SubjectCode WHERE StudentID = " + studentID;
            command = new MySqlCommand(query, mySqlConn);
            mySqlConn.Open();
            dReader = command.ExecuteReader();
            ...
            for (i = 0; i < tracker; i++)
            {
                ...
                units.Add(new Unit(subjectCode[i], subjectDesc[i], grade[i]));
            }
